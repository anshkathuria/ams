				...
				if($getStudents = $mysqli->query ("SELECT * from `students-$year-$sec`"))
				{
					...
					while($studentObj = $getStudents->fetch_assoc())
					{
						...
						<tr>
							...
							$attdPercent = ($tot==0)?100:($totAttd/$tot)*100;
							?>
							<td><?=$totAttd ?></td>
							<td <?=($attdPercent<69.0)?'style="background-color:#ea7d79"' : (($attdPercent<75.0)?'style="background-color:#f0ad4e"' : '') ?>>
								<?=($tot==0)?'-':number_format($attdPercent,2,'.','') ?>
							</td>
						</tr>
						<?
					}
